...
CConfirmDlg::CConfirmDlg(const CTBMOperate::Operation* operation, CWnd* pParent)
	: CDialog(CConfirmDlg::IDD, pParent),
	m_resize(this)
{
...
BOOL CConfirmDlg::OnInitDialog()
{
	...
	if (m_operation != NULL)
	{
		...
		if (m_operation->object != CTBMOperate::Operation::TBOBJ_LZL)
		{
			unique_ptr<vector<CString> > img(new vector<CString>());
			if (m_operation->object == CTBMOperate::Operation::TBOBJ_THREAD)
...
void CConfirmDlg::OnBnClickedButton1()
{
	...
	CString url;
	if (m_operation->object == CTBMOperate::Operation::TBOBJ_THREAD) // 主题
		url = _T("http://tieba.baidu.com/p/") + m_operation->tid;
	else if (m_operation->object == CTBMOperate::Operation::TBOBJ_POST) // 帖子
