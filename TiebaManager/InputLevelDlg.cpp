...
{
	m_param->m_operator = LevelOperator(m_operatorCombo.GetCurSel());
	CString tmp;
...
CConditionParam* CInputLevelDlg::SetLevelCondition(CConditionParam* _param)
{
	auto param = (CLevelParam*)_param;
	BOOL release = FALSE;
	if (param == nullptr)
	{
		param = new CLevelParam();
		release = TRUE;
	}

	CInputLevelDlg dlg(param);
	if (dlg.DoModal() == IDOK)
		return param;
